jQuery(document).ready(function ($) {
    ...
    */
    if ( jQuery ( 'body' ).hasClass( 'wp-admin' ) && jQuery( 'body' ).hasClass( 'post-type-shop_order' ) && jQuery( '.wp-list-table' ).length > 0 ){
    	jQuery(document).ready(function(){
    		jQuery('.button.tips.download').each(function(){
    				jQuery(this).html('Compress Files');
    		});
    	});		
    	jQuery(document).on('click','.button.tips.download',function(e){
    		e.preventDefault();
    		$_thiobj = jQuery(this);
			$_thiobj.html('Compressing Files');   		
    		var vars = [], hash;
    		var hashes = jQuery(this).attr('href').split('=');
			...
			if ( debug ) console.log( "ajaxurl::: " + ajaxurl ) ;
				...
				jQuery.post(ajaxurl, {
			     ...
			     function ( data, status ) {
			     	...
			     	var result = data;
					if ( result != null ){
						jQuery(result.link).insertAfter($_thiobj);
						$_thiobj.html('Download');
						$_thiobj.remove();
						//jQuery(result.link).trigger("click");
						//jQuery(document).find('.redydownload').trigger("click");
					}
				}
			);			
    	});
    }
    jQuery(document).on('click','.redydownload',function(){
    		//jQuery(this).remove();
    });
});
